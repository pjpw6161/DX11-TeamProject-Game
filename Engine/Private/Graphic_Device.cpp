...
HRESULT CGraphic_Device::Initialize(const GRAPHIC_DESC& GraphicInfo, _Inout_ ID3D11Device** ppDeviceOut, _Inout_ ID3D11DeviceContext** ppDeviceContextOut)
{
	// LPDIRECT3DTEXTURE9
	...
	if (FAILED(Ready_SwapChain(GraphicInfo)))
		return E_FAIL;
	...
	if (FAILED(Ready_DepthStencilRenderTargetView(GraphicInfo.iWinSizeX, GraphicInfo.iWinSizeY)))
		return E_FAIL;
	...
	ViewPortDesc.Width = static_cast<_float>(GraphicInfo.iWinSizeX);
	ViewPortDesc.Height = static_cast<_float>(GraphicInfo.iWinSizeY);
...
HRESULT CGraphic_Device::Ready_SwapChain(const GRAPHIC_DESC& GraphicInfo)
{
	...
	/*텍스쳐(백버퍼)를 생성하는 행위*/
	SwapChain.BufferDesc.Width = GraphicInfo.iWinSizeX;
	SwapChain.BufferDesc.Height = GraphicInfo.iWinSizeY;
...
CGraphic_Device * CGraphic_Device::Create(const GRAPHIC_DESC& GraphicInfo, ID3D11Device ** ppDevice, ID3D11DeviceContext ** ppContext)
{
	CGraphic_Device* pInstance = new CGraphic_Device();

	if (FAILED(pInstance->Initialize(GraphicInfo, ppDevice, ppContext)))
	{
