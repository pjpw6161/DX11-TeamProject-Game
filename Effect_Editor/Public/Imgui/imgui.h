...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // start a new Dear ImGui frame, you can submit any command from this point until DrawEditor()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by DrawEditor(). If you don't need to render data (skipping rendering) you may call EndFrame() without DrawEditor()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the draw data. You can then get call GetDrawData().
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after DrawEditor() and until the next call to NewFrame(). this is what you have to render.
    ...
    IMGUI_API bool          SliderRadian(const char* label, float* v_rad, float v_degrees_min = -360.0f, float v_degrees_max = +360.0f, const char* format = "%.0f deg", ImGuiSliderFlags flags = 0);
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is updated during the frame. valid before DrawEditor(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
struct ImGuiIO
{
    ...
    //------------------------------------------------------------------
    // Output - Updated by NewFrame() or EndFrame()/DrawEditor()
    ...
    int         MetricsRenderVertices;              // Vertices output during last call to DrawEditor()
    int         MetricsRenderIndices;               // Indices output during last call to DrawEditor() = number of triangles * 3
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(const ImVec2& clip_rect_min, const ImVec2& clip_rect_max, bool intersect_with_current_clip_rect = false);  // DrawEditor-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImDrawData
{
    bool                Valid;              // Only valid after DrawEditor() is called and before the next NewFrame() is called.
