...
public:
	/* 그래픽 디바이스의 초기화. */
	HRESULT Initialize(const GRAPHIC_DESC& GraphicInfo, _Inout_ ID3D11Device** ppDevice,
...
private:
	...
	HRESULT Ready_SwapChain(const GRAPHIC_DESC& GraphicInfo);
	HRESULT Ready_BackBufferRenderTargetView();
	HRESULT Ready_DepthStencilRenderTargetView(_uint iWinCX, _uint iWinCY);

public:
	static CGraphic_Device* Create(const GRAPHIC_DESC& GraphicInfo, _Inout_ ID3D11Device** ppDevice,
