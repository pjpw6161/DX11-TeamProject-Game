...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text <= 2000 || wrap_enabled)
    {
        ...
        // DrawEditor (we don't hide text after ## in this end-user function)
...
void ImGui::Bullet()
{
    ...
    // DrawEditor and stay on same line
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // DrawEditor text. We currently only render selection when the widget is active or while scrolling.
    ...
    if (render_cursor || render_selection)
    {
        ...
        // DrawEditor text (with cursor and selection)
    ...
    {
        // DrawEditor text only (no selection, no cursor)
        if (is_multiline)
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        // DrawEditor Cursor + preview on Hue Wheel
        float cos_hue_angle = ImCos(H * 2.0f * IM_PI);
        float sin_hue_angle = ImSin(H * 2.0f * IM_PI);
        ...
        // DrawEditor SV triangle (rotated according to hue)
    ...
    // DrawEditor cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // DrawEditor tab label, process close button
...
// DrawEditor text label (with custom clipping) + Unsaved Document marker + Close Button logic
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    // DrawEditor text label (with clipping + alpha gradient) + unsaved marker
